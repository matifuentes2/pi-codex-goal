...
import {
  __require
} from "./index-a1s5r901.js";

// src/index.ts
import { Type } from "typebox";
var GOAL_TOOL_NAME_GUIDANCE = "Call each goal tool by the name exposed in your available tool list. In pi that is usually get_goal, create_goal, and update_goal; in bridged MCP runs it may be a namespaced variant such as pi__get_goal, pi__create_goal, or pi__update_goal. Do not assume display, history, or transcript tool names are callable unless they appear in your tool list.";
...
async function textResult(text, goal, includeCompletionBudgetReport = false) {
  const { goalToolResponse } = await import("./format-tea9sgxj.js");
  return {
    content: [{ type: "text", text }],
    details: { ...goalToolResponse(goal, includeCompletionBudgetReport), error: null }
  };
}
function registerLazyGoalTools(pi, getController) {
  pi.registerTool({
    name: "get_goal",
    label: "Get Goal",
    ...
    async execute() {
      ...
      const { toToolText } = await import("./format-tea9sgxj.js");
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const { createGoal, replaceGoal } = await import("./state-m9sqzkxc.js");
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      const controller = await getController();
      const result = controller.completeGoal("tool", ctx);
      if (!result.ok || !result.goal) {
        throwToolError(result.message);
      }
      const { toToolText } = await import("./format-tea9sgxj.js");
...
function registerLazyGoalCommand(pi, getController) {
  pi.registerCommand("goal", {
    ...
    async handler(args, ctx) {
      const controller = await getController();
      const { handleGoalCommand } = await import("./commands-jb0hnech.js");
      await handleGoalCommand(pi, {
...
function src_default(pi) {
  ...
  let pendingSessionTree = null;
  let replayedStartupEvents = false;
  const replayStartupEvents = async (nextController) => {
    if (replayedStartupEvents)
      return;
  ...
  const getController = async () => {
    if (controller)
      return controller;
    controllerPromise ??= import("./goal-runtime-controller-feg5d7cc.js").then(async (mod) => {
  ...
  try {
    unsubscribeStartEvent = pi.events.on("pi-codex-goal:start", async (payload) => {
      ...
      const { replaceGoal } = await import("./state-m9sqzkxc.js");
      const { compactContinuationPrompt } = await import("./prompts-eszdw255.js");
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (controller)
      await controller.onSessionStart(event, ctx);
  });
  pi.on("session_tree", async (event, ctx) => {
    pendingSessionTree = { event, ctx };
    if (controller)
      await controller.onSessionTree(event, ctx);
  });
  ...
  pi.on("before_agent_start", async (event, ctx) => (await getController()).onBeforeAgentStart(event, ctx));
  ...
  pi.on("tool_execution_end", async (event, ctx) => (await getController()).onToolExecutionEnd(event, ctx));
  ...
  pi.on("session_before_compact", async (event, ctx) => (await getController()).onSessionBeforeCompact(event, ctx));
  pi.on("session_compact", async (event, ctx) => (await getController()).onSessionCompact(event, ctx));
  pi.on("session_shutdown", async (event, ctx) => {
    unsubscribeStartEvent?.();
    latestContext = null;
    if (controller)
      await controller.onSessionShutdown(event, ctx);
  });
}
export {
